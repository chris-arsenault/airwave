...
export function DevicePill() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <div
          className={`w-2 h-2 rounded-full ${playing ? "bg-emerald-400" : "bg-[var(--color-text-secondary)]"}`}
        />
            ...
            .map((device) => {
              ...
              return (
                <button
                  ...
                  disabled={!selectable}
                  className={`w-full flex items-center gap-3 px-4 py-3 text-left text-sm transition-colors ${deviceButtonClass(selectable, device.id === activeDeviceId)}`}
                >
                  <SpeakerIcon />
                  <div className="flex-1 min-w-0">
                    <div className="truncate font-medium">
                      {device.name}
                      {isSlave && (
                        <span className="text-[10px] text-orange-300 ml-1.5">(follower)</span>
                      )}
                    </div>
...
function deviceButtonClass(selectable: boolean, isActive: boolean): string {
  if (!selectable) return "opacity-40 cursor-not-allowed";
  if (isActive) return "bg-[var(--color-accent)]/10 text-[var(--color-accent)]";
